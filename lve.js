...
const lve = (_name) => {
	...
	if (typeof _name == "string") {
		...
		switch (_name) {
			...
			default: {
				retArray = lve.root.cache.selectorKeyword[_name];
				break;
			}
		}
	}
...
class CreateSession {
	...
	getEasingData(_attr) {
		...
		var t = ani.count[_attr] * 1000 / 60 || 1,
			b = tar,
			c = tar_goal - tar,
					...
					if (typeof data == "object") {
						...
						for (let j in _data) {
							...
							if (item.type == "image" && j == "src") {
								item[j] = newAttrVal;
								// 이미지 미리불러오기
								fn.preloadData(item, "img", (self, element) => {
									self.element = element;
								});
								continue;
							}
	...
	// 객체가 현재 위치를 직접 이동합니다
	// 여러대의 객체가 선택되었을 시 가장 최초 객체를 선택
	follow(tarObjName, relativePosition) {
	...
	animate(data, duration, easing, callback) {
		if (!data)
			return;
...
lve.root.fn.adjustJSON = (data, obj) => {
	// 새로운 객체로 생성
...
lve.root.fn.preloadData = (tarObj, tag, callback) => {
	let tmpObj = document.createElement(tag);

	tmpObj.onload = () => {
		callback(tarObj, tmpObj);
	};
...
lve.root.fn.isRotateVisible = (item) => {
	let vars = lve.root.vars,
...
lve.root.fn.setRotateObject = (item) => {
...
lve.root.fn.canvasReset = (item = { style: {}, relative: {} }) => {
	let vars = lve.root.vars,
		initSetting = vars.initSetting,
		usingCamera = vars.usingCamera,
